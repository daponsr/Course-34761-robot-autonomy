...
class MyOccupancyGrid(Node):
    ...
    def odom_callback(self, msg):
        ...
        self.robot_pose_odom = msg.pose.pose
    ...
    def scan_callback(self, msg):
        ...
        if(self.robot_pose_odom is None):
        ...
        _, _, yaw = self.euler_from_quaternion(self.robot_pose_odom.orientation)
        if(yaw < 0):
            yaw += 2 * np.pi
        ...
        xy = self.polar_to_cartesian(polar, self.robot_pose_odom.position.x, self.robot_pose_odom.position.y, yaw)
        xy[:, 0] = (xy[:, 0] + self.width // 2) / self.resolution
        xy[:, 1] = (xy[:, 1] + self.height // 2) / self.resolution
        xy = xy.astype(int)
        xo = int((self.robot_pose_odom.position.x + self.width // 2) / self.resolution)
        yo = int((self.robot_pose_odom.position.y + self.height // 2) / self.resolution)
        # update the grid
